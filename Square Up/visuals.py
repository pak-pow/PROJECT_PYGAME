...
class Debris:
    ...
    def draw(self, surf, cam):
        sx, sy = cam.world_to_screen(self.wx, self.wy)
        if self.type == "blood":
            pygame.draw.ellipse(surf, (100, 0, 0), (sx - 10*self.scale, sy - 5*self.scale, 20*self.scale, 10*self.scale))
        elif self.type == "scorch":
            pygame.draw.circle(surf, (20, 20, 20), (sx, sy), 25 * self.scale)
...
class VisualManager:
    def __init__(self):
        ...
        self.casings = []
        self.debris = []
        self.ghosts = [] # New list for dash traces
        self.fonts = {
